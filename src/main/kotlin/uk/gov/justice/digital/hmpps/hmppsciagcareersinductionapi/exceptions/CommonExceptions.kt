...
class NotFoundException(var offenderId: String) : Exception("Readiness profile does not exist for offender $offenderId"), Supplier<Throwable> {
  override fun get(): Throwable {
    throw NotFoundException(offenderId)
  }
}

class InvalidStateException(var offenderId: String) : Exception("Readiness profile is in an invalid state for  $offenderId"), Supplier<Throwable> {
  override fun get(): Throwable {
